...
!***********************************************************************
	subroutine rtm_2d_single_shot_extrapolation_allocate(nnx, nnz, &
		...
		!allocate wavefiled
		allocate(us1%u(-npad+1:nnz+npad, -npad+1:nnx+npad), stat=ierr)
		if(ierr.ne.0)then
			write(*,*)"Can not allocate working memory about us1, stop!!!"
			stop
		endif

		allocate(us2%u(-npad+1:nnz+npad, -npad+1:nnx+npad), stat=ierr)
		if(ierr.ne.0)then
			write(*,*)"Can not allocate working memory about us2, stop!!!"
			stop
		endif

		allocate(us3%u(-npad+1:nnz+npad, -npad+1:nnx+npad), stat=ierr)
		if(ierr.ne.0)then
			write(*,*)"Can not allocate working memory about us3, stop!!!"
			stop
		endif

		allocate(ur1%u(-npad+1:nnz+npad, -npad+1:nnx+npad), stat=ierr)
		if(ierr.ne.0)then
			write(*,*)"Can not allocate working memory about ur1, stop!!!"
			stop
		endif

		allocate(ur2%u(-npad+1:nnz+npad, -npad+1:nnx+npad), stat=ierr)
		if(ierr.ne.0)then
			write(*,*)"Can not allocate working memory about ur2, stop!!!"
			stop
		endif

		allocate(ur3%u(-npad+1:nnz+npad, -npad+1:nnx+npad), stat=ierr)
		if(ierr.ne.0)then
		...
		if(iflag_src .eq. 1)then
			...
			allocate(ust%u(-npad+1:nnz+npad, -npad+1:nnx+npad, ns), stat=ierr)
			if(ierr.ne.0)then
...
!***********************************************************************
	subroutine rtm_extrapolation_2d_forward_variable_update()
...
!***********************************************************************
	subroutine rtm_extrapolation_2d_backward_variable_update(iflag_src)
...
!***********************************************************************
	subroutine rtm_extrapolation_2d_backward_Reinitiallization()
